...
(function($){
	...
	$.fn.dcDrilldown = function(options) {
		...
		var defaults = {
			...
			showCount		: false,
			classCount		: 'dd-count',
			classIcon: 'dd-icon ui-icon ui-icon-circle-triangle-e',
			linkType: 'backlink',
			resetText		: 'All',
			headerTag		: 'h3',
		...
		return this.each(function(options){
			...
			function setUpDrilldown(){
				...
				totalWidth += 'px';
				var itemHeight = $('li:first', $dcDrilldownObj).height()
				if (itemHeight == 0) itemHeight = 38

				// Get height of largest sub menu
				var objUl = $('ul',$dcDrilldownObj.parentNode);
		...
		function updateHeader(obj, html){
			if(defaults.includeHdr == true){
			    if ($('h3', obj).length) {
			        $('h3', obj).html(html);
			        $('h3', obj).children("span").removeClass("ui-icon ui-icon-circle-triangle-e");
				} else {
